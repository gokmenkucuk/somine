...
class AuthRepository {
  ...
  /// Sign in with Apple
  Future<({UserCredential? userCredential, bool isNewUser})> signInWithApple() async {
    try {
  ...
  Future<bool> _handleUserLogin(User firebaseUser) async {
    try {
      ...
      debugPrint('🔵 [AuthRepository] isNewUser: $isNewUser (existingUser: ${existingUser != null})');
  ...
  Future<void> _createDemoContent(String userId, List<CategoryModel> categories) async {
    try {
      ...
      for (final demo in demoUrls) {
        try {
          ...
          debugPrint('✅ [AuthRepository] Demo item created: ${metadata?.title ?? url}');
          hourOffset++;
        } catch (e) {
  ...
  Future<void> _reauthenticateUser(User user) async {
    try {
      ...
      if (providerData.isEmpty) {
        debugPrint('⚠️ [AuthRepository] No provider data, skipping reauthentication');
        return;
      }
      
      final providerId = providerData.first.providerId;
      debugPrint('🔵 [AuthRepository] Reauthenticating with provider: $providerId');
