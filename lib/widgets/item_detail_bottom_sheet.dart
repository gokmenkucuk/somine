...
import '../core/utils/auth_image_provider.dart';
...
class _ItemDetailBottomSheetState extends State<ItemDetailBottomSheet> {
  ...
  void _resolveImageSize() {
    ...
    if (isApiStorageUrl(imageUrl)) {
      resolveImageHeaders(imageUrl).then((headers) {
        if (!mounted) return;
  ...
  Widget _buildFallbackHeader(IconData icon) {
    ...
    return Container(
      ...
      padding: const EdgeInsets.only(top: 48),
      color: context.colors.surfaceWhite,
      child: Center(child: Icon(icon, size: 56, color: context.colors.primary)),
    );
  }
  ...
  Widget build(BuildContext context) {
    ...
    final headerHeight =
        (MediaQuery.of(context).size.height * headerRatio) + statusBarHeight;
